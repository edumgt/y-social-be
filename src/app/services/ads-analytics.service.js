...
class AdsAnalyticsService {
    async updateDailyAnalytics() {
        ...
        for (const ad of ads) {
            ...
            // which checks if the start date of the ad's schedule is less than or equal to the current date (today).
            // (ad.schedule_end ? today <= ad.schedule_end : true). 
            // It uses a ternary operator (? :) to check if the ad has an end date specified.
            ...
            // If both conditions are true, the code inside the if statement will be executed.
